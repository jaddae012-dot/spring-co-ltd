        ...
        {items.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm transition-colors ${
              item.active
                ? "bg-emerald-700/50 text-slate-100 border border-emerald-500/50"
...
const teachingItems: PortalItem[] = [
  { label: "Overview", icon: "≡", active: true },
  { label: "Course Planner", icon: "▤" },
  { label: "Lessons", icon: "▥" },
  { label: "Calendar", icon: "◷" },
...
const workspaceItems: PortalItem[] = [
  { label: "Profile", icon: "◫" },
...
export default function TutorPortalSidebar() {
  const menuItems = [...teachingItems, ...workspaceItems];
