...
export default function FastCleanersAdminLoginPage() {
  const router = useRouter();
  const [id, setId] = useState("");
  const [pin, setPin] = useState("");
  const [error, setError] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [redirectTo, setRedirectTo] = useState("/fast-cleaners/admin/dashboard");

  useEffect(() => {
    router.prefetch("/fast-cleaners/admin/dashboard");
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 pt-24 text-slate-100">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-2xl shadow-cyan-950/30">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Sign in to manage cleaners, jobs, and bookings.</p>
        </div>
        ...
        <div className="mt-6 text-center text-sm text-slate-400">
          <p>
            ...
            <Link href="/fast-cleaners/login" className="font-medium text-cyan-400 hover:text-cyan-300">
